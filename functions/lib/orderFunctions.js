...
exports.extendETA = exports.cancelOrder = exports.updateOrderStatus = void 0;
...
exports.updateOrderStatus = (0, https_1.onCall)(async (request) => {
    try {
        ...
        if (role !== 'staff' && role !== 'admin') {
            throw new https_1.HttpsError('permission-denied', 'Only staff or admin can update order status');
        }
        ...
        if (role === 'staff' && orderData.cafeteriaId !== userData.cafeteriaId) {
            throw new https_1.HttpsError('permission-denied', 'Staff can only update orders for their cafeteria');
        }
        ...
        if (!validTransitions[currentStatus] || !validTransitions[currentStatus].includes(data.newStatus)) {
            throw new https_1.HttpsError('invalid-argument', 'Invalid status transition');
        }
 ...
 */
exports.cancelOrder = (0, https_1.onCall)(async (request) => {
    try {
        // 1. Auth check
        if (!request.auth) {
            throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
        ...
        if (!canCancel) {
            throw new https_1.HttpsError('permission-denied', 'Cannot cancel this order');
        }
 ...
 */
exports.extendETA = (0, https_1.onCall)(async (request) => {
    try {
        ...
        const role = userData.role;
        if (role !== 'staff' && role !== 'admin') {
            throw new https_1.HttpsError('permission-denied', 'Only staff or admin can extend ETA');
        }
...
//# sourceMappingURL=orderFunctions.js.map